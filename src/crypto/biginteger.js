...
(function(exports) {
  ...
  BigInteger.parse = function(s, base) {
    ...
    function expandExponential(str) {
      ...
      return str.replace(/^([+\-])?(\d+)\.?(\d*)[eE]([+\-]?\d+)$/, (x, s, n, f, c) => {
        c = +c;
        const l = c < 0;
        let i = n.length + c;
        x = (l ? n : f).length;
    ...
    const parts = new RegExp(`^([+\\-]?)(${prefixRE})?([0-9a-z]*)(?:\\.\\d*)?$`, 'i').exec(s);
    if (parts) {
      let sign = parts[1] || '+';
  ...
  BigInteger.prototype.divRem = function(n) {
    ...
    while (b_index) {
      ...
      if (part._s === 0) {
        ...
        const xlen = part._d.length; const
          ylen = a._d.length;
  ...
  (function() {
    ...
    (function() {
      let i; let
        fn;
      const unary = 'toJSValue,isEven,isOdd,sign,isZero,isNegative,abs,isUnit,square,negate,isPositive,toString,next,prev,log'.split(',');
      const binary = 'compare,remainder,divRem,subtract,add,quotient,divide,multiply,pow,compareAbs'.split(',');
